...
/// Canonical Students are every 10th ID (sim-010, sim-020, ...). Delegates to
/// the production suffix helper; production mints the routine durably, so no
/// production caller of this predicate remains.
pub fn is_student_id(id: &str) -> bool {
    caelum_core::commute::numeric_id_suffix(id).is_multiple_of(10)
}
