...
fn collect_due_events(
    ...
    loop {
        let Some((&time, _)) = scheduler.buckets.iter().next() else {
            break;
        };
        if time.0 > horizon {
            break;
...
fn worker_next_activity(
    ...
    if !outbound_resolved && !outbound_arrived {
    ...
    if outbound_arrived && !return_resolved && !returned_home {
