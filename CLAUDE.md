...
The central rule: **Rust owns gameplay state; `createGameRuntime()` (`src/runtime/createGameRuntime.ts`) owns the frontend runtime boundary.** Rust's `GameSnapshot` is the durable/core authority and `PresentationUpdate` is the ordinary Rust host wire; the runtime folds every update into the flat `GameState` presentation view — not a save model — plus local `UiState`, publishes subscriptions, calls the host backend, and mounts the GPU display host (`createWebGpuHost`). Rust owns the occupancy/crowding/traffic/aggregate-demand/service projection; individual citizen rendering and the lateness/growth shipped overlays are removed. Svelte renders from runtime snapshots and emits intents back — it is never a second source of truth. See `docs/architecture.md` for the full description.

**Intent flow:** a Svelte component calls a `RuntimeController` method (e.g. `setTool`, `handleTileClick`, `togglePause`) → the runtime applies local UI helpers or dispatches a gameplay intent/tick to the active backend → `commit()` swaps in the new snapshot/UI state, re-renders the frame, and publishes a `RuntimeSnapshot` to subscribers. `createWebGpuHost` owns the `requestAnimationFrame` loop and admits ticks to the backend at ≤10 host ticks/sec while unpaused (one in flight, wall-delta clamped), interpolating vehicle presentation between observed snapshots. The load-bearing standalone Bevy ECS population from HPA-347 is live in `caelum-core`.

**Simulation core is Rust (`crates/caelum-core`)**, independent of Svelte and Tauri. `GameEngine` (`engine.rs`) is the facade both hosts drive: `tick(delta_seconds)` advances game time (real seconds scaled by the current speed), and `dispatch(GameIntent)` applies one player action (build / paint area / transit edit / speed / pause). A tick runs `trips::tick_trips_with_objectives`, which **substeps** the delta at growth-wave boundaries — firing due growth waves (`growth::apply_due_growth_waves`) and re-evaluating objectives/metrics after each substep — bounded by a `max_tick_substeps` cap. Every step takes an immutable `GameSnapshot` and returns a new one. `GameEngine` owns shell + topology + ECS world/schedule as one commit unit: the live shell's `sims` are empty (latent citizens live only in the ECS world), the exact-time scheduler wakes only due events, and `applied = shell_changed || population_changed`. The explicit durable snapshot reconstructs schema-v11 sims; one `project_update` consumes `PopulationAggregates` (durable- or ECS-built, parity-tested). HPA-348 owns route-choice batching; HPA-640 delivered the WebGPU renderer/cadence cutover. Browser and Tauri hosts both wrap this same `GameEngine`.
...
- `render/` — WebGPU geometric presentation. `createWebGpuHost` (`src/runtime/createWebGpuHost.ts`) owns the GPU surface, input wiring, resize, the rAF loop with ≤10 Hz tick admission, and device-loss forwarding; `src/render/webgpu/` owns painter-order batches, the instanced vehicle draw, interpolation, and culling; `boardTransform.ts` keeps the tile↔pixel mapping (`tileSize = 32`). The runtime creates the real `<canvas>` and attaches it to `GameCanvas.svelte`'s host element; Svelte owns HUD/panels and the DOM map-text overlay. Canvas2D gameplay rendering is deleted.
