...
- `transit::assign_vehicle_costed` already demonstrates the vehicle-cost and route-validity rules, although HPA-624 needs an atomic multi-bus initial deployment rather than repeated single-vehicle dispatches.
- `Route.legs` is already the cyclic service itinerary used by live vehicle movement.
- HPA-622's `traffic::RoadFlow`, `effective_road_step_seconds`, and `effective_road_path_seconds` are the current source of congestion-aware bus timing.
...
This means the existing generic route editor remains generic while its creation policy becomes mode-specific:
...
Route editing stays structural and must not create, remove, or reposition fleet.
...
- schema fixtures/tests/documentation that assert v6.

Old development cities disappear. Do not add migration code, fallback namespaces, aliases, dual readers, or serde defaults for the new route field.
...
For a connected bus route, calculate one cycle by walking the existing cyclic `Route.legs` in order and summing the same effective path-step durations used by live bus movement:

```text
road step seconds = step.travel_seconds * congestion_multiplier(current road flow)
```

Track paths are not relevant to this bus-only slice. Zero-step synthetic legs contribute zero because live vehicle movement skips them.
...
Put the Rust calculation in a small bus-specific module, for example `bus_service.rs`, rather than growing `transit.rs` further or introducing a generic operations framework.
...
If assigned fleet is zero, current headway is unavailable. This is the HPA-624 meaning of “actual headway”: it reflects the travel time the currently assigned evenly-spaced fleet would produce under current congestion. It is **not** measured departure history and does not imply bunching detection.

The UI may recompute the same small formula from the snapshot for presentation using the already mirrored traffic-flow constants. Rust remains authoritative for deployment count and cost. Tests must lock the Rust and TypeScript calculations to the same representative vectors.
...
Keep `route_lifecycle::is_route_operational(active, legs)` as the structural active/connected check because `assign_vehicle_costed` and other lifecycle code use it before fleet exists. Instead, narrow passenger eligibility in `router::active_services`:
...
Do not turn “No fleet” into a new persisted route lifecycle state.
...
- current round-trip time is positive and derivable from the live route paths.
...
Repeated host dispatches would expose partial purchases, recompute against intermediate snapshots, and make deterministic spacing a UI concern. One Rust mutation keeps cost, IDs, and placement atomic without creating a fleet manager.
...
Resolve the offset across the route's legs and path steps using the same congestion-adjusted road-step duration used for the round-trip calculation. The located cursor initializes:
...
`step_progress` is the fraction of the located effective step already traversed. Skip zero-duration/zero-step legs. Because `i < N`, offsets are always in `[0, roundTripSeconds)`.
...
This proves deterministic spacing on unequal route legs without adding departure clocks or holding.
...
The bus route editor therefore validates route geometry without blocking Save on the future fleet purchase. Fleet affordability is shown only when the player deploys the fleet from the Lines row.
...
- Metro rows keep their current compact controls with no empty service block;
...
Do not add charts, history, per-vehicle rows, plus/minus fleet buttons, route visibility toggles, or advanced operations navigation.
...
It may reuse `selectTrafficFlow(state)` and the mirrored congestion constants from `src/domain/traffic.ts` to derive:
...
This helper is presentation-only. Deployment remains a Rust intent and must independently derive the authoritative required count from the same snapshot semantics.
...
- unequal-duration legs produce deterministic cursor offsets in stable vehicle-ID order;
...
- bus service metrics match representative Rust vectors for free-flow and congested paths;
...
### TypeScript-owned fleet calculation with repeated `AssignVehicle`

Rejected. It makes the host responsible for authoritative cost/count/placement and can partially mutate the route. Rust already owns simulation, cost policy, IDs, and vehicle cursors.

### Automatic fleet resizing whenever headway or congestion changes

Rejected. It would require purchase/withdrawal policy, passenger-safe removal, and likely vehicle re-spacing. HPA-624 only needs the initial deployment loop; resizing is a later gameplay decision.
...
- Adding buses after initial deployment, withdrawal, reassignment, purchase inventory, depots, crews, maintenance, or breakdowns.
- Automatic headway/fleet optimization.
