...
- **Accepted:** parallel same-axis one-way endpoint links must be prevented and inherited ones must be normalized away.
...
A connection-time guard alone is also insufficient. A lateral endpoint link can be created while both roads are TwoWay and later become a parallel-one-way link after direction edits. `CycleRoadDirection` does not call `connect_neighbor_endpoints`; it changes `one_way` and then runs automatic-junction refresh/canonicalization. The topology invariant therefore needs two parts:

1. **prevent** new lateral links when both adjacent tiles are already parallel one-way lanes;
2. **normalize away** any existing reciprocal lateral link once both endpoint tiles become parallel one-way lanes.
...
## 2. Parallel one-way lanes never retain lateral links

Add one small private predicate shared by connection and canonicalization:
...
In `connect_neighbor_endpoints`, use the predicate instead of the current opposite-direction-only check. Any adjacent one-way lanes on the same axis skip a perpendicular connection.

### Normalize inherited links

Extend `canonicalize_authored_roads` so a reciprocal connection is retained only when it is both topologically reciprocal **and not** a lateral parallel-one-way link.

When collecting each tile's snapshot for canonicalization, carry `tile.one_way` alongside its connection list. For each heading, read the neighbor's current `one_way` and filter with the same predicate before writing the canonical connection list back.

This makes the invariant robust when:

- a new Dual carriageway is placed next to an existing OneWay;
- an unwanted link already existed between TwoWay endpoints and both endpoint cells are later changed to the same one-way axis;
- authoring order differs.
...
- an existing lateral endpoint edge is removed once both endpoint tiles become parallel one-way lanes;
...
The spacing policy is explicitly tool-scoped. Direction edits and Dual placement can produce geometry the standalone OneWay tool would refuse; Task 2 guarantees that such geometry does not retain forbidden lateral parallel-one-way links.
...
Do not rely on the 3-tile rule for graph correctness. Parallel-one-way lateral links are normalized independently of how the geometry was authored.
